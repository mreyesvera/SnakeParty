/*
    ...
    This code defines the events for the components of the 
    register changes form. It allows to validate input values
    and save the user's changes into the database once the
    ...
    function () {
        $("#registerForm").submit(
            ...
            function (event) {
                ...
                let params = "datebirth=" + $("#datebirth").val() +
                    "&color=" + $("#color").val();
        ...
        function registered(response) {
            if (response === -1) { // if there was no error
                $("#response").html("Edited Successfully").css(
                    "color", "#53b844"
                );
